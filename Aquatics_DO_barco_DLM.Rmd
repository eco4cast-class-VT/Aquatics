...
DLM <- nimbleCode({
 ...
 sd_add ~ dunif(0, 100)
 beta_0 ~ dnorm(0,sd=5) #dnorm(1.11,sd=0.01)
 beta_1 ~ dnorm(0,sd=5) #dnorm(0.53,sd=0.1)
 beta_x ~ dnorm(0,sd=5) #dnorm(1.73,sd=0.1)
...
for(t in 2:nt){
  ...
  for(m in 1:num_particles){
    ...
    x[t,m] <- x[t-1,m] +  beta_0 + beta_1 * Temp[m,t] - beta_x * x[t-1,m] + rnorm(1, mean = 0, sd = sd_add)
