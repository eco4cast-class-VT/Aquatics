...
```{r}
colnames(DO.det) <- seq(1,time2,1)
forecast_output_april = as_tibble(DO.det) 
forecast_output_april = forecast_output_april %>% mutate(ensemble = seq(1,nrow(DO.det),1)) %>% pivot_longer(cols=1:time2, names_to="day", values_to="oxygen" )
forecast_dates = tibble(time = seq(as.Date(start_date),as.Date(end_date), by = "days"), day = seq(1,time2,1))
forecast_output_april$day = as.numeric(forecast_output_april$day)
forecast_output_april =  left_join(forecast_output_april,forecast_dates,by="day")
forecast_output_april = forecast_output_april %>% mutate(siteID = "BARC") %>%
  mutate(forecast = 1) %>% select(-day)
forecast_output_april
```

Write forecast output to csv
```{r}
write.csv(forecast_output_april, file="aquatics-2021-04-01-VTaquatics.csv")
```
