...
forecastN <- function(IC, theta, kdo, sd_add, n){
...
N.det <- forecastN(IC = IC[prow_IC,"x"],
                   theta = params[prow,"theta"],
                   kdo = params[prow,"kdo"],
                   sd_add = params[prow,"sd_add"],  ## process error
                   n = Nmc)

ci <- apply(N.det, 2, quantile, c(0.025, 0.5, 0.975))
ci <- t(ci)
ci <- as.data.frame(ci) %>% mutate(time = seq(as.Date("2021-03-01"), as.Date("2021-03-07"),1))
colnames(ci) <- c("lower","median","upper","time")

  ggplot() +
    xlim(c(as.Date("2021-02-01"),as.Date("2021-03-07"))) +
  geom_line(data= DO_pred, aes(x = date, y = mean)) +
  geom_line(data= ci, aes(x = time, y = median)) +  
  geom_ribbon(data= DO_pred, aes(x = date, ymin = lower, ymax = upper), alpha = 0.2, color = "lightblue", fill = "lightblue") +
    geom_ribbon(data= ci, aes(x = time, ymin = lower, ymax = upper), alpha = 0.2, color = "lightblue", fill = "lightblue") +
  geom_point(data = daily_data, aes(x = time, y = oxygen), shape=21, color="darkblue",size=1.5) + 
    scale_shape_identity() + labs(x = "Date", y = "DO (mg/L)", title = "DO model") 
```
...
```{r}
colnames(N.det) <- seq(1,7,1)
forecast_output = as_tibble(N.det) 
forecast_output = forecast_output %>% mutate(ensemble = seq(1,nrow(N.det),1))
forecast_output = forecast_output %>% pivot_longer(cols=1:7, names_to="day", values_to="oxygen" )
forecast_dates = tibble(time = seq(as.Date("2021-03-01"),as.Date("2021-03-07"), by = "days"), day = seq(1,7,1))
