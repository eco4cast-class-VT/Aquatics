...
#remvoing first year with missing data and adding in NAs that NEON didn't have 
#make list of all dates for desired time frame: choosing data since 2019 since data before 2019 is very spotty, and Feb 2021 is most recent data from NEON 
dates_for_modelfit <- data.frame( Date = seq(as.Date("2019-01-01"), as.Date("2021-02-28"), by="days")  )

#merge two data frame best on date so that dates w/out NAs are now in data frame just NA 
daily_data <- left_join(dates_for_modelfit, daily_data, by = "Date") %>% 
  mutate(siteID = "BARC") #this is just to assign a site to Dates that don't have data for plotting below

#replace uncertainty NAs with 0
daily_data$DOuncert_no_NAs <-ifelse(is.na(daily_data$dissolvedOxygenExpUncert), daily_data$DOuncert_no_NAs <- 0, daily_data$DOuncert_no_NAs <-          daily_data$dissolvedOxygenExpUncert)
```
...
#Selecting Temp data to match filtered daily DO above (see lines 82-87)
daily_temp <- left_join(dates_for_modelfit, daily_temp, by = "Date")%>% 
  mutate(siteID = "BARC") #this is just to assign a site to Dates that don't have data for plotting below
...
Run nimble model using arrhenius and Michaelis-menten equations for temp + DO
```{r}
...
RandomWalk <- nimbleCode({
  ...
  #Kt ~ dnorm(1.2, sd=0.1)
  sd_add ~ dunif(0, 100)
 # kdo ~ dnorm(0.002, sd=0.001)
 # k20 ~ dnorm(1.1, sd=0.1) 
  #theta ~ dnorm(1.05, sd=0.01)
    #### Process Model
  for(t in 2:n){
   # Kt[t] <- k20 * theta ^(Tmin[t]-20)
    pred[t] <- x[t-1]#Kt[t] * (DO[t-1]/kdo + DO[t-1])
...
constants <- list(n = length(DO),
                  x_ic = 8,
                  sd_ic = 0.1,
                  sd_obs = 0.1,#daily_data$dissolvedOxygenExpUncert ,
                  Tmin = Tmin,
                  DO = DO)#,
                  #kdo = 0.0014,
                  #k20 = 1.2,
...
for(i in 1:nchain){
  ...
  inits[[i]] <- list(sd_add = sd(diff(y.samp),na.rm=T),
                     x = DO)#,
                     #kdo = rnorm(1, 0.0014, 0.001)#,
                     #k20 = rnorm(1, 1.1, 0.1),
                     #theta = rnorm(1, 1.05, 0.01))
}
...
#effectiveSize(nimble_burn)
#gelman.diag(nimble_burn)  ## determine convergence
...
```{r}
Tmin <- daily_temp$tsdWaterTempMean
# replace NA's with mean value to avoid having NA's in initialization
#Tmin_init <- data.frame(Tmin) 
#Tmin_init = Tmin_init %>% mutate(Tmin = if_else(is.na(Tmin)==T,mean(Tmin,na.rm=T),Tmin))

DO <- daily_data$dissolvedOxygen
 # replace NA's with mean value to avoid having NA's in initialization
DO_init <- data.frame(DO) 
DO_init = DO_init %>% mutate(DO = if_else(is.na(DO)==T,mean(DO,na.rm=T),DO))

sd_obs <- daily_data$DOuncert_no_NAs
...
DLM <- nimbleCode({
  ...
  beta_1 ~ dnorm(0, sd = 1000)
  #beta_x ~ dnorm(0, sd = 1000)
  
    #### Process Model
  for(t in 2:n){
    pred[t] <- x[t-1] +  beta_0 + beta_1 * Tmin[t-1]
    x[t] ~ dnorm(pred[t], sd = sd_add)
  }
